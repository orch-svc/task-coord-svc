...
// LoggerFactory.getLogger(TcsJobRegisterListener.class);
//
...
// TCSDriver.getContext()
// .getBean("defaultMessageConverter");
// final TcsProducer producer =
// TCSDriver.getContext().getBean(TcsProducer.class);
// return new TcsJobRegisterListener(messageConverter, producer);
// }
//
// protected TcsJobRegisterListener(MessageConverter messageConverter,
// TcsProducer producer) {
...
// final TcsListenerContainerFactory factory =
// TCSDriver.getContext().getBean(TcsListenerContainerFactory.class);
//
// listenerContainer = factory.createListenerContainer(this,
// Arrays.asList(TCSConstants.TCS_REGISTER_TASK_QUEUE,
// TCSConstants.TCS_QUERY_TASK_QUEUE));
...
// }
//
// private void processQueryJobInstanceRequest(Message message, Channel channel,
// final Object object)
...
// if (jobDAO != null) {
// final QueryJobInstanceResponse result = new
// QueryJobInstanceResponse(jobDAO.getInstanceId(),
...
// */
// TcsCtrlMessageResult<?> processRegisterJob(JobSpecRegistrationMessage
// message) {
// final JobDefinition job = message.getJobSpec();
//
// final JobDefinitionCycleDetector cycleDetector = new
// JobDefinitionCycleDetector(job);
// if (cycleDetector.detectCycle()) {
// LOGGER.error("Cycle detected in Job definition for job: {}",
// job.getJobName());
...
// LOGGER.error("Exception in JobDefinition.validateSteps() for Job: " +
// job.getJobName(), ex);
...
// LOGGER.error("Exception in TcsJobExecSubmitListener.processRegisterJob()",
// e);
// return null;
// }
//
